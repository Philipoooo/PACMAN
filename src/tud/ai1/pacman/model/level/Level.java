...
public class Level {
    ...
    public boolean isSolid(int x, int y) {
    	...
    	if(map[x][y].isSolid() == true) {
    		return true;
    	} else if(getWidth() > y | getHeight() > x) {
    		return true;
    	} else {
    		return false;}
    	}
    ...
    if(map[p.x-1][p.y].isWall() == false) {
    	Ausgabe.add(new Point((p.x)-1, (p.y)));
    } else if(map[p.x+1][p.y].isWall() == false) {
    	Ausgabe.add(new Point((p.x)+1, (p.y)));
    } else if(map[p.x][p.y+1].isWall() == false) {
    	Ausgabe.add(new Point((p.x), (p.y+1))); }
    	 else if(map[p.x][p.y-1].isWall() == false) {
    	    	Ausgabe.add(new Point((p.x), (p.y-1)));
    	   }
    Point[] array = Ausgabe.toArray(new Point[Ausgabe.size()]);
    return array;
